...
# UPDATED CHANNELS DATA (Ab sirf 3 channels hain)
REQUIRED_CHANNELS = [
...
async def start_handler(message: types.Message):
    ...
    if await is_user_joined(user_id):
        ...
        await message.answer("❌ Niche diye gaye channels join karein tabhi bot chalega:", reply_markup=builder.as_markup())
...
async def check_cb(callback: types.CallbackQuery):
    if await is_user_joined(callback.from_user.id):
        ...
        await callback.answer("❌ Saare channels join nahi kiye!", show_alert=True)
