...
def get_main_menu():
    ...
    kb.add(InlineKeyboardButton(text="🔗 My Referral Link", callback_data="my_ref"))
    return kb
...
async def view_methods(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup(row_width=1)
    # Match these IDs with your /setmethod commands
    ...
    await call.message.answer("<b>Unlock Methods:</b>", reply_markup=kb)
...
async def process_unlock(call: types.CallbackQuery):
    ...
    if user['points'] >= 5:
        ...
        if method_data:
            ...
            # Copies the exact video/file from your admin chat
            await bot.copy_message(chat_id=call.from_user.id, from_chat_id=ADMIN_ID, message_id=method_data['msg_id'])
            await call.message.answer("<b>✅ Method Sent Successfully!</b>")
        else:
            await call.answer("❌ This method is currently unavailable.", show_alert=True)
    else:
...
async def set_method(message: types.Message):
    ...
    if not m_id or not message.reply_to_message:
        return await message.reply("Reply to a Video/File/Text with: <code>/setmethod 1</code>")
    
    methods_db.update_one({"m_id": m_id}, {"$set": {"msg_id": message.reply_to_message.message_id}}, upsert=True)
    await message.reply(f"✅ Method {m_id} linked successfully!")
...
if __name__ == '__main__':
    ...
    # CRITICAL: This line forces the bot to disconnect other sessions
